...
with tab1:
    ...
    days_int  = [int(d) for d in dm['Day'].tolist()]
    residuals = [float(a - p) for a, p in zip(dm['WAIT'].tolist(), y_pred.tolist())]
    ...
    fig2.add_trace(go.Scatter(x=days_int, y=dm['WAIT'].tolist(),
        name='WAIT Actual',
        line=dict(color='#2b6cb0', width=3)), row=1, col=1)
    fig2.add_trace(go.Scatter(x=days_int, y=y_pred.tolist(),
        name='WAIT Predicted (GB)',
        line=dict(color='#c53030', width=2, dash='dash')), row=1, col=1)
    fig2.add_trace(go.Bar(x=days_int, y=residuals,
        name='Residuals (°C)',
        marker_color=['#c53030' if r < 0 else '#276749' for r in residuals],
        marker_line_width=0), row=2, col=1)
    fig2.add_hline(y=0, line_dash='dash', line_color='gray', row=2, col=1)
    fig2.update_layout(
        plot_bgcolor='#ffffff', paper_bgcolor='#f0f4f8',
        font=dict(color='#1a202c'), height=520,
        title=f'Model Fitting | R²={r2*100:.2f}% | Mean Error={errors.mean():.3f}°C | Max Error={errors.max():.2f}°C',
        legend=dict(x=0.01, y=0.99)
    )
    y_min = min(dm['WAIT'].min(), min(y_pred)) - 0.5
    y_max = max(dm['WAIT'].max(), max(y_pred)) + 0.5
    fig2.update_yaxes(title_text='WAIT (°C)', row=1, col=1, range=[y_min, y_max])
    fig2.update_yaxes(title_text='Residual (°C)', row=2, col=1)
    fig2.update_xaxes(title_text='Day from SOR', row=2, col=1, dtick=10)
...
""")
    ...
    start_fixed    = datetime.combine(start_date, datetime.min.time())
